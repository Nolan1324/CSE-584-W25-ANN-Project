...
def build_tree(
    data: npt.NDArray, attr_names: List[str], atomics: List[Tuple[Atomic, int]], params: TreeAlgoParams
) -> PartitionTree:
    ...
    return _build_tree_helper(
        data, attr_names, list(sorted(atomics, key=lambda x: x[1], reverse=True)), params
    )
...
def _get_partitions_from_tree_helper(
    node: PartitionTree, cur_ranges: Dict[str, Range]
...
def get_example_tree() -> PartitionTree:
    atomics = [
        (Atomic("x", Operator.GTE, 500), 10),
        (Atomic("x", Operator.GTE, 100), 9),
        (Atomic("y", Operator.GTE, 300), 8),
    ]
